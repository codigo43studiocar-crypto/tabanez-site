  ...
  async function abrirRelease(r) {
    ...
    try {
      ...
      if (typeof json.views === "number") {
        setReleases((prev) =>
          prev.map((item) =>
            item.id === r.id ? { ...item, views: json.views } : item
          )
        );
        setSelecionado((prev) =>
    ...
    <div className="w-full">
      ...
      <section className="w-full bg-neutral-900 text-white py-10 md:py-14">
        <div className="section">
          <h1 className="text-3xl md:text-5xl font-extrabold">Clipping</h1>
          <p className="text-base md:text-lg mt-2 text-gray-200 max-w-2xl">
            Seleção de matérias oficiais de imprensa de Carlos Tabanez.
          </p>
          <p className="text-xs md:text-sm mt-3 text-gray-300">
            Para modo lista, acesse{" "}
            <a
              href="/clipping/lista"
          ...
          <>
            <div className="grid gap-6 md:grid-cols-3">
              {releasesPagina.map((r) => (
                <article
                  key={r.id}
                  className="bg-white rounded-card shadow-card border border-gray-100 overflow-hidden flex flex-col hover:-translate-y-1 hover:shadow-lg transition"
                >
                  {r.imagem ? (
